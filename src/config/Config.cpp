...
Config Config::load(const std::string& path) {
    ...
    try {
        ...
        if (j.contains("uid")) cfg.uid = j["uid"];
        if (j.contains("server_url")) cfg.server_url = j["server_url"];
        if (j.contains("tasks_folder")) cfg.tasks_folder = j["tasks_folder"];
        if (j.contains("results_folder")) cfg.results_folder = j["results_folder"];
